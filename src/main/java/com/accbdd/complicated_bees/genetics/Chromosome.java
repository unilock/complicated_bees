...
public class Chromosome {
    ...
    public Chromosome() {
        ...
        if (ComplicatedBees.GENE_REGISTRY != null)
            for (Map.Entry<ResourceKey<IGene<?>>, IGene<?>> entry : ComplicatedBees.GENE_REGISTRY.entrySet()) {
                genes.put(entry.getKey().location(), ComplicatedBees.GENE_REGISTRY.get(entry.getKey().location()));
            }
    }
    ...
    public IGene<?> getGene(ResourceLocation id) {
        return this.genes.getOrDefault(id, ComplicatedBees.GENE_REGISTRY.get(id));
    }
    ...
    public static Chromosome deserialize(CompoundTag tag) {
        ...
        for (Map.Entry<ResourceKey<IGene<?>>, IGene<?>> entry : ComplicatedBees.GENE_REGISTRY.entrySet()) {
            ...
            if (!geneData.equals(new CompoundTag())) {
                ...
                genes.put(id, Objects.requireNonNull(ComplicatedBees.GENE_REGISTRY.get(id)).deserialize(geneData));
